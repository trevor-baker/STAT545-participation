...
```r
#this just keeps the rows in song that we have album for, but it doesn't actually add the album
time %>% 
...
```r
#diff is that this adds all columns from 'album'
time %>% 
...
```r
#if you just wanted the album column, then you'd have to select columns from 'album' via pipe
```
...
```r
album %>% 
  full_join(time, by = c("song", "artist_name")) #same result, different column order
```
...
- leaves zero rows, even by end of first anti_join, but this is the idea
...
union(
  mutate(y, day = "Day 1"),
  mutate(z, day = "Day 2")
) #works but could remove rows, don't want to leave that option open
```
...
bind_rows(
  mutate(y, day = "Day 1"),
  mutate(z, day = "Day 2")
) #better because will never remove rows
```
