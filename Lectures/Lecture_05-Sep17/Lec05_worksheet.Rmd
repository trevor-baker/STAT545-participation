...
```
#install.packages("tidyverse") #already done
...
gapminder %>% 
  filter(year == 2007) %>% 
  mutate(continent = fct_infreq(continent)) %>% #fct_infreq - reorder factors by their count frequency
...
ggplot(mauna, aes(y = month)) +
  geom_line(aes(month,conc)) + ylab("conc_CO2")
```
...
```{r, fig.width = 5, fig.height = 2}
p +
  geom_point(colour = "green3", alpha=0.1)
```
...
p + 
  coord_polar() + 
  ylab("Concentration CO2")

# #doesn't work
# mauna$mm <- months(mauna$month)
# ggplot(mauna, aes(x=mm, y=conc)) + coord_polar() + geom_point()
