...
```r
dplyr::select(gapminder, year, lifeExp, country)
```
...
```r
# Better way:
select(gapminder, country:lifeExp) #colon between 1st and last columns
```
...
```r
# compare
select(gapminder, cont=continent, everything()) #can do it in select, but will reorder things
```
...
```r
select(gapminder, everything(), cont=continent, gdp=gdpPercap) #this does not move cont to the end, just ensures that all columns are included.
```

```
## # A tibble: 1,704 x 6
##    country     cont   year lifeExp      pop   gdp
##    <fct>       <fct> <int>   <dbl>    <int> <dbl>
...
```r
rename(gapminder, cont=continent) #rename it in place, no reorder
```
...
# alter the below to include 2 "pipes"
  # first select columns, then arrange
  # don't need to id the dataframe after first line
  # the pipe puts the previous line into the first argument of the next function
gapminder %>%
...
gapminder %>%
  filter(pop>100000000) %>%
  filter(continent=="Asia") #longer way
```
...
```r
gapminder %>%
  filter(pop>100000000 & continent=="Asia") #shorter way
```
...
```r
gapminder %>%
  filter(pop>100000000, continent=="Asia") #same same
```
...
gapminder %>%
  ...
  mutate(gdpBill=round(gdp/1e9,2))
```
...
gapminder %>%
  ...
  transmute(gdpBill=round(gdp/1e9,2))
```
...
gapminder %>%
  ...
  filter(year==1952) %>%
  mutate(lifeExp=if_else(lifeExp==68.75,70,lifeExp)) #this leaves just one row
```
...
```r
gapminder %>%
  mutate(lifeExp=if_else(country=="Canada" & year==1952 & lifeExp==68.75, 70, lifeExp)) #this keeps all rows - this is correct
```
