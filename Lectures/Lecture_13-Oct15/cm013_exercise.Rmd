...
```{r}
p_scales <- ggplot(gapminder, 
                   aes(gdpPercap, lifeExp)) +
              geom_point(aes(colour=pop), alpha=0.4)
...
p_scales +
    scale_x_log10() +
    scale_colour_continuous(trans  = "log10", 
                            breaks = 10^(5:9), #only label breaks at 1e5 to 1e9
                            limits=c(1e1,1e10) ) + #extend colour scale from 10 to 10 billion
...
p_scales +
    scale_x_log10(labels=scales::dollar_format()) +
    scale_color_distiller( trans   = "log10", #need to use _distiller not _brewer
                        breaks  = 10^(1:10),
                        labels  = scales::comma_format(),
                        palette = "Spectral") +
    scale_y_continuous(breaks=10*(1:10))
...
ggplot(iris, aes(Sepal.Width, Sepal.Length)) +
    ...
    theme(axis.title=element_text(face = "bold") ,
          strip.background = element_rect("lightblue"))
```
...
p %>% 
    ggplotly() %>% 
    htmlwidgets::saveWidget("test_plotly.html")
```
...
# p %>% 
#     ggplotly() %>% 
#     plotly_json() %>% head() #this is ugly output, will not show it in html
```
